...
assert_exit_code "1.3 v-flag" 0 maxbin-rs -v
...
if [ "$BINS_PT09" = "$BINS_DDASH" ]; then
  pass "4.5 double-dash+hyphens identical bins"
else
  fail "4.5 double-dash+hyphens bins differ"
fi
...
if [ -n "${READS1:-}" ] && [ -n "${READS2:-}" ]; then
  ...
  assert_exit_code "7.8 legacy-multi-reads" 0 \
    maxbin-rs -contig "$CONTIGS" -reads "$READS1" -reads2 "$READS2" -out "$D/multi_reads/test"
...
if [ -n "${READS1:-}" ]; then
  ...
  assert_exit_code "7.9 legacy-reads-list" 0 \
    maxbin-rs -contig "$CONTIGS" -reads_list "$D/reads_list/list.txt" -out "$D/reads_list/test"
